...
createTable <- function(k1.file,k2.file,empirical.data.file,clust.file)
{
  ...
  all.values <- list(k1.phi,k2.phi,clust,empirical.data) %>% purrr::reduce(left_join,by="GeneID")
  return(all.values)
...
plotSpecies <- function(species.df,cub=c("Phi_Shared_Mutation","Phi_Variable_Mutation"))
{
  ...
  levels(species.df$CUB) <- c("ROC (Shared Mutation Bias)","ROC (Variable Mutation Bias)")
  emp.vs.pred <- ggplot(species.df,aes(x=Empirical,y=Predicted)) + geom_pointdensity(size=0.5,show.legend = F) + 
               scale_x_log10(labels = trans_format("log10", math_format(10^.x))) + 
               scale_y_log10(labels = trans_format("log10", math_format(10^.x))) +
               theme_cowplot() +
               viridis::scale_color_viridis(option = "magma") + 
  ...
  k1.vs.k2 <- ggplot(species.df.wide,aes(x=Phi_Shared_Mutation,y=Phi_Variable_Mutation,color=Cluster)) + 
              ...
              xlab("ROC (Shared Mutation Bias)") +
              ylab("ROC (Variable Mutation Bias)") +
...
{
  cor.table <- species.df %>% group_by(CUB) %>% summarize(Correlation=cor.test(Empirical,Predicted,use="complete.obs",method="spearman")$estimate,P = cor.test(Empirical,Predicted,use="complete.obs",method="spearman")$p.value)
  cor.table <- cor.table %>% mutate(Species=unique(species.df$Species))
  return(cor.table)
}


compareFits<- function(species.df,cub=c("Phi_Shared_Mutation","Phi_Variable_Mutation"),...)
{
  title <- cleanSpeciesLabels(unique(species.df$Species))
...
treeVsCorrelation <- function(tree,cor.df)
{
  tree.plot <- ggtree(tree) + xlim_tree(500)
  tree.plot <- tree.plot + geom_tiplab()
  corr.plot <- facet_plot(tree.plot,panel="Correlation Shifts\nVarMut vs. ConstMut",data=cor.df,geom=geom_segment,aes(x=0,xend=ROC_Difference,y=y,yend=y,color=Major.clade),size=4)
  return(corr.plot)
}
...
df <- pmap(list(runs.k.1,runs.k.2,emp.data,mixtures),createTable) %>% bind_rows(.id="Species")
...
gc.sig <- target.species.gc %>%
          ...
          mutate(ks.results=map2(`2`,`1`,~ks.test(.x$GC3,.y$GC3))) %>% 
          mutate(tidied=map(ks.results,tidy),`1` = map(`1`, nrow),`2` = map(`2`, nrow)) %>% 
          unnest(tidied,`1`,`2`) %>% 
          select(-ks.results)

gc.stats <- gc.median %>% left_join(gc.sig,by="Species") %>%
            mutate(label=str_c(as.character(round(Median.Diff,2)),
                               paste0("p = ",as.character(round(p.value,2)),sep="",collapse=""),sep="\n")) %>%
...
gc3.violin.clust <- ggplot(target.species.gc,aes(x=Cluster,y=GC3,fill=Cluster)) + 
              ...
              geom_bracket(data=gc.stats,aes(label = label,xmin=Group1,xmax=Group2,y.position=0.85),step.group.by="Species",inherit.aes=F) +
              geom_text(data=gc.stats.long,aes(label=Count,x=Cluster,y=Min-0.025)) +
...
tree.plot <- ggtree(target.tree,size=2, ladderize=F) +
  ...
  ylab("Mya") +
  theme_dendrogram(plot.margin=margin(20,4,60,4)) 


emp.pred.gc3 <- plot_grid(tree.plot,phi.shared,gc3.violin.clust,nrow=3,ncol=1,labels=c("A","","B"),rel_heights = c(1,1,1),axis = "tblr",align="hv")

emp.pred.gc3

ggsave("Figures/Figure_1.pdf",plot = emp.pred.gc3,width = 18,height = 14)
...
ggsave2("Figures/Figure_2.pdf",plot=phi,width = 16,height = 14)
```
...
calb <- compareGC3ToClusterFreq("Results/Clustering/candida_albicans.max.cds","Data/GC/candida_albicans.max.cds","Data//gff/candida.albicans.max.cds",gff.id="ID",chr.reg="chr[0-9R]",title="C. albicans Chr. 6",chr.to.plot = "chr6")
...
sel.ratio <-sel.ratio %>% group_by(Species,AA) %>% 
  mutate(Min.Mean_1 = rank(Mean_1),Min.Mean_2 = rank(Mean_2)) %>% 
  mutate(Optimal_1 = ifelse(Min.Mean_1 == 1,"Yes","No"),Optimal_2=ifelse(Min.Mean_2 == 1,"Yes","No")) %>%
  ungroup()
